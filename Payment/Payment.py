...
async def get_payment(id: int, db: db_dependency, current_user: User = Depends(get_current_user)):
    pedido = db.query(models.Pedido).filter(models.Pedido.ID_Comprador == id).first()
    ...
    for item in itens:
        ...
        listItens.append({"id": db_Item.ID, "title": db_Item.Nome, "quantity": item.Quantidade, "currency": "BRL", "unit_price": db_Item.Preco})
    ...
    payment_data = {
        ...
        "back_urls": {
            ...
            "failure": "127.0.0.1:8000",
            "pending": "127.0.0.1:8000"
        },
        "auto_return": "all"
    }
